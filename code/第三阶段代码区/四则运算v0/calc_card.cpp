...
#include<sys/time.h>
#include <QFile>
Calc_card::Calc_card(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::Calc_card)
{
    ui->setupUi(this);
    ...
    if(time_counter_display<0){
        ...
        if(num_question <= 0){
            ...
            case QMessageBox::Help:
                //打开帮助文档
                QMessageBox tt(QMessageBox::Question,"dd","bangzhu");
                tt.exec();
                break;
            }
        }
        else{
            ui->question->setText(temp[5-num_question]);
...
void Calc_card::on_submit_clicked()
{
    ...
    if(answer==NULL){
        ...
        num_question--;
        bool answer_result=check_userAnswer(ANS[5-num_question-1]);
        //check(answer)
